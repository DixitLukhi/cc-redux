    ...
    const columns = [
        ...
        },
        {
            header: 'Amount', field: (row) => {
                return <div className="text-lg font-semibold text-yankeesBlue">{row.due_amount}</div>
            }
        },
        {
            header: 'Deposit', field: (row) => {
                return <i className={classNames('pi', { 'text-green-500 pi-check-circle': row.cycle_deposit_status, 'text-red-500 pi-times-circle': !row.cycle_deposit_status })}></i>;
            }
        },
        {
            header: 'Withdraw', field: (row) => {
                return <i className={classNames('pi', { 'text-green-500 pi-check-circle': row.cycle_withdraw_status, 'text-red-500 pi-times-circle': !row.cycle_withdraw_status })}></i>;
            }
        },
        {
            header: '', field: (row) => {
                return <div onClick={(e) => { e.stopPropagation(); setPayerData(row); setIsPayPopUpOpen(true); }} className="relative text-base font-semibold text-white inline-block bg-[#8FB50B] rounded-lg px-3 py-2">{row.cycle_deposit_status === false ? "Pay" : "Withdraw"}</div>
            }
        },
    ];
    return (
